...
<img src="../figures/tioga/Tioga-Mod2-Ex1.png" width="250"/>
...
<img src="../figures/tioga/Tioga-Mod2-Ex2.png" width="250"/>
</details>
...
<img src="../figures/tioga/Tioga-Mod2-Ex4.png" width="250"/>
...
<img src="../figures/tioga/Tioga-Mod2-Ex5.png" width="550"/>
...
For `srun -N1 -n16 -t1 --cpu-bind=rank mpi` and `srun -N1 -n48 -t1 --cpu-bind=rank mpi`, you will get output of the format

```
[username2@ip-10-0-0-11 ~]$ srun -N1 -n16 -t1 --cpu-bind=rank mpi
gpu-st-g4dnmetal-2 Task   1/ 16 running on 1 CPUs: 1
gpu-st-g4dnmetal-2 Task   3/ 16 running on 1 CPUs: 3
gpu-st-g4dnmetal-2 Task   8/ 16 running on 1 CPUs: 8
(...)
```

where `Task N/16` runs on CPU `N`. On the other hand, you'll see

```
[username2@ip-10-0-0-11 ~]$ srun -N1 -n49 -t1 --cpu-bind=rank mpi
srun: error: Unable to allocate resources: Requested node configuration is not available
```

This is because there are only 48 cores per node --- up to 48 ranks only.
...
<img src="../figures/tioga/Tioga-Mod2-Ex6.png" width="550"/>
...
<img src="../figures/tioga/Tioga-Mod2-Ex7.png" width="550"/>
...
```
[username2@ip-10-0-0-11 ~]$ srun -t1 -N1 -n2 -c3 --cpu-bind=core mpi
gpu-st-g4dnmetal-2 Task   1/  2 running on 6 CPUs: 24-26,72-74
gpu-st-g4dnmetal-2 Task   0/  2 running on 6 CPUs: 0-2,48-50
[username2@ip-10-0-0-11 ~]$ srun -t1 -N1 -n2 -c3 --cpu-bind=thread mpi
gpu-st-g4dnmetal-2 Task   0/  2 running on 3 CPUs: 0-2
gpu-st-g4dnmetal-2 Task   1/  2 running on 3 CPUs: 24-26
```

Behavior here is similar to what was shown on Tioga. When we bind
each task to 3 threads, each task is assigned to 3 CPUs (3 hardware
threads). When we bind each task to 3 cores, each task is assigned to
6 CPUs (6 hardware threads, since each of the 3 cores has 2 threads.)
...
<img src="../figures/tioga/Tioga-Mod2-Ex8.png" width="550"/>
...
<img src="../figures/tioga/Tioga-Mod2-Ex9.png" width="550"/>
</details>
...
Here are the results of the above commands on AWS:

```
[username2@ip-10-0-0-11 topo-xml]$ srun -t1 -N1 -n4 --cpu-bind=map_cpu:7,5,10,6 mpi
gpu-st-g4dnmetal-2 Task   0/  4 running on 1 CPUs: 7
gpu-st-g4dnmetal-2 Task   1/  4 running on 1 CPUs: 5
gpu-st-g4dnmetal-2 Task   2/  4 running on 1 CPUs: 10
gpu-st-g4dnmetal-2 Task   3/  4 running on 1 CPUs: 6
[username2@ip-10-0-0-11 topo-xml]$ srun -t1 -N1 -n3 --cpu-bind=map_cpu:7,5,10,6 mpi
gpu-st-g4dnmetal-2 Task   0/  3 running on 1 CPUs: 7
gpu-st-g4dnmetal-2 Task   1/  3 running on 1 CPUs: 5
gpu-st-g4dnmetal-2 Task   2/  3 running on 1 CPUs: 10
[username2@ip-10-0-0-11 topo-xml]$ srun -t1 -N1 -n8 --cpu-bind=map_cpu:7,5,10,6 mpi
gpu-st-g4dnmetal-2 Task   2/  8 running on 1 CPUs: 10
gpu-st-g4dnmetal-2 Task   0/  8 running on 1 CPUs: 7
gpu-st-g4dnmetal-2 Task   1/  8 running on 1 CPUs: 5
gpu-st-g4dnmetal-2 Task   7/  8 running on 1 CPUs: 6
gpu-st-g4dnmetal-2 Task   4/  8 running on 1 CPUs: 7
gpu-st-g4dnmetal-2 Task   5/  8 running on 1 CPUs: 5
gpu-st-g4dnmetal-2 Task   3/  8 running on 1 CPUs: 6
gpu-st-g4dnmetal-2 Task   6/  8 running on 1 CPUs: 10
```

With only 3 tasks (`-n3`), cores 5, 7 and 10 are used; 6 is not needed.
With 8 tasks, cores 5-7 and 10 are all used; each is assigned 2 tasks
...
<img src="../figures/tioga/Tioga-Mod2-Ex10.png" width="550"/>
</details>
...
<img src="../figures/tioga/Tioga-Mod2-Ex11.png" width="550"/>
...
Creating the mask:
```
[username2@ip-10-0-0-11 ~]$ srun -t1 -N1 -n1 hwloc-calc --taskset --physical-output NUMAnode:0.core:4-8
0x1f00000000001f0
```

Checking the mask does in fact code for cores 4-8:

```
[username2@ip-10-0-0-11 ~]$ srun -t1 -N1 -n1 hwloc-calc 0x1f00000000001f0 --intersect core
4,5,6,7,8
```
...
The full command will be `srun -N1 -n2 --cpu-bind=mask_cpu:0x1f00000000001f0 mpi`:

```
[username2@ip-10-0-0-11 ~]$ srun -N1 -n2 --cpu-bind=mask_cpu:0x1f00000000001f0 mpi
gpu-st-g4dnmetal-2 Task   0/  2 running on 10 CPUs: 4-8,52-56
gpu-st-g4dnmetal-2 Task   1/  2 running on 10 CPUs: 4-8,52-56
```
...
```
[username2@ip-10-0-0-11 ~]$ srun -N1 -n1 -t1 hwloc-calc --taskset --physical-output NUMAnode:1
0xffffff000000ffffff000000
```
...
```
[username2@ip-10-0-0-11 ~]$ srun -N1 -t1 --cpu-bind=mask_cpu:0xffffff000000ffffff000000 mpi
gpu-st-g4dnmetal-2 Task   0/  1 running on 48 CPUs: 24-47,72-95
```
...
1. Creating a mask for each of the first three cores on NUMA 0:

```
[username2@ip-10-0-0-11 ~]$ srun -N1 -n1 -t1 hwloc-calc --taskset NUMAnode:0.core:0
0x1000000000001
[username2@ip-10-0-0-11 ~]$ srun -N1 -n1 -t1 hwloc-calc --taskset NUMAnode:0.core:1
0x2000000000002
[username2@ip-10-0-0-11 ~]$ srun -N1 -n1 -t1 hwloc-calc --taskset NUMAnode:0.core:2
0x4000000000004
```

2. From above, the mask for the 2nd NUMA node is `0xffffff000000ffffff000000`.

3. Binding the four tasks using these masks:

```
[username2@ip-10-0-0-11 ~]$ srun -N1 -n4 -t1 --cpu-bind=mask_cpu:0x1000000000001,0x2000000000002,0x4000000000004,0xffffff000000ffffff000000 mpi
gpu-st-g4dnmetal-2 Task   3/  4 running on 48 CPUs: 24-47,72-95
gpu-st-g4dnmetal-2 Task   0/  4 running on 2 CPUs: 0,48
gpu-st-g4dnmetal-2 Task   1/  4 running on 2 CPUs: 1,49
gpu-st-g4dnmetal-2 Task   2/  4 running on 2 CPUs: 2,50
```
...
```
[user2@ip-10-0-0-11 ~]$ srun -N1 -n8 -t1 --cpu-bind=map_cpu:22,23,46,47 mpi
gpu-st-g4dnmetal-2 Task   0/  8 running on 1 CPUs: 22
gpu-st-g4dnmetal-2 Task   1/  8 running on 1 CPUs: 23
gpu-st-g4dnmetal-2 Task   2/  8 running on 1 CPUs: 46
gpu-st-g4dnmetal-2 Task   3/  8 running on 1 CPUs: 47
gpu-st-g4dnmetal-2 Task   4/  8 running on 1 CPUs: 22
gpu-st-g4dnmetal-2 Task   5/  8 running on 1 CPUs: 23
gpu-st-g4dnmetal-2 Task   6/  8 running on 1 CPUs: 46
gpu-st-g4dnmetal-2 Task   7/  8 running on 1 CPUs: 47
```
...
```
[username2@ip-10-0-0-11 ~]$ srun hwloc-calc --taskset --physical-output NUMAnode:0.core:21-23
0xe00000000000e00000
[username2@ip-10-0-0-11 ~]$ srun hwloc-calc --taskset --physical-output NUMAnode:0.PU:1-3
0x3000000000002
[username2@ip-10-0-0-11 ~]$ srun hwloc-calc --taskset --physical-output NUMAnode:1.core:1-5
...
```
[username2@ip-10-0-0-11 ~]$ srun hwloc-calc 0xe00000000000e00000 --intersect core
21,22,23
[username2@ip-10-0-0-11 ~]$ srun hwloc-calc 0xe00000000000e00000 --intersect PU --physical-output
21,69,22,70,23,71
[username2@ip-10-0-0-11 ~]$ srun hwloc-calc 0x3000000000002 --intersect core
0,1
[username2@ip-10-0-0-11 ~]$ srun hwloc-calc 0x3000000000002 --intersect PU --physical-output
48,1,49
[username2@ip-10-0-0-11 ~]$ srun hwloc-calc 0x3e00000000003e000000 --intersect core
25,26,27,28,29
[username2@ip-10-0-0-11 ~]$ srun hwloc-calc 0x3e00000000003e000000 --intersect PU --physical-output
25,73,26,74,27,75,28,76,29,77
```
...
```
[username2@ip-10-0-0-11 ~]$ srun -N1 -n3 --cpu-bind=mask_cpu:0xe00000000000e00000,0x3000000000002,0x3e00000000003e000000 mpi
gpu-st-g4dnmetal-2 Task   2/  3 running on 10 CPUs: 25-29,73-77
gpu-st-g4dnmetal-2 Task   0/  3 running on 6 CPUs: 21-23,69-71
gpu-st-g4dnmetal-2 Task   1/  3 running on 3 CPUs: 1,48-49
```
